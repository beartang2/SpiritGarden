...
public class InteractBoxController : MonoBehaviour
{
    ...
    public float distance = 2f;   // 캐릭터와 큐브 사이의 고정 거리
    public LayerMask groundLayer; // 마우스가 클릭할 수 있는 레이어 (예: 땅)
    public Collider cubeCollider;   // 큐브의 콜라이더

    private Vector3 lastDirection; // 큐브의 마지막 방향을 저장
    private float fixedY;          // 고정된 Y축 값
    public float colliderActiveTime = 0.1f; // 콜라이더를 잠시 활성화하는 시간
